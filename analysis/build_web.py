...
论文面向会读论文的人; 这个页面面向要做决定的普通人——项目目标里的
"给当代的普通人一个参考"最终得靠它兑现, 它也是唯一面向使用者的入口。

页面在浏览器里跑完整算法(而非查预制表), 因此任何输入都能算, 且与 buyrent 包同源
(等价性由 tests/test_web_calculator.py 逐位断言)。
产出两份, 同一个模板:
...
def payload() -> dict:
    """两个口径必须与 Python 完全一致, 否则网页会给出与论文不同的答案。
