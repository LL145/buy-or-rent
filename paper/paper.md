...
时间约定：所有现金流按年频、在年初发生（租金预付；月供合并为年供），期末 $T$ 结算。年供与贷款余额按标准的期末等额本息公式计算，但在年初支付——买方因此比逐月还款略早付出现金，这个近似的量级很小，方向对买方偏保守。
...
三个风格化画像（由 `python3 analysis/cases.py` 生成，参数见表）：
...
四类城市画像、两类租房投资者（保守型：存款/国债/理财，真实收益 1.5%；均衡型：含权益或全球配置，真实 3%），持有 10 年，估值按各城相对自身 25 年常态的偏离条件化（所有画像都落在"高估组"——即便经过大跌，中国城市的租售比仍未回到自己 2000 年代的常态；各城"常态"的取值是假设而非数据，其敏感性见本节末尾）：
...
**估值分组是假设，不是数据。** 上表的条件化需要每个城市"过去 25 年的常态租金收益率"，而中国没有公开的城市级长序列可查。本文取的常态值（一线与强二线 2.4%、二线 2.8%、三四线 3.0%）是判断，不是测量，因此必须报告它对结论的影响。常态值只通过估值三分位进入模型，所以敏感性可以直接按分组算：下表给出每个城市落入中间组的**门槛**（常态租金收益率低于该值即不再属于高估组），以及三种分组下的保守投资者胜率。
...
两点读法。第一，**二线的常态假设 2.8% 距其门槛 2.74% 只有 6 个基点**——成都、武汉过去 25 年的常态租金收益率若略低于 2.8%，该档就落入中间组，胜率从 46% 升到 54%（区间 [46, 63]）。第二，所有敏感性都朝同一个方向：放松高估假设只会让买方更有利，正文的高估组口径是这组结论里**对买方最保守**的一端。因此 8.3 节"二线可以买"的判断不依赖这个假设（两种分组下胜率都在四六开之间、均不越过 55%），而"一线偏租"的判断依赖于"一线过去 25 年常态租金收益率高于 2.05%"——读者若掌握本地长序列，应以之替换本文的假设值重算。
...
1. **体制依赖（可切换，且已检验）。** 历史分布混合了金本位、战争管制、战后信贷扩张与低利率时代，1950 年后那轮利率长下行的一次性重估不可重演。算法支持限定样本期（`History(since=1950)`），本文默认全样本以保守为先。
...
5. **买的是一套房，不是全国指数。** JST 的房价是全国指数，而个人承担的是单套住宅相对指数的特异性波动，后者远大于指数本身，且完全不在窗口数据里。算法提供开关（`run(..., idio_sd=)`），但本文**不估计**这个波动率——全国指数估不出它，使用者应以本地重复销售证据自行设定。把它开到年化 0%–15%：
...
8. **幸存者偏差。** 进入窗口统计的 16 国全部是活到 2020 年的发达经济体，且 JST 只收录有连续长序列的国家。经历过革命、没收、恶性通胀后重建序列的国家要么不在库中，要么其最坏年份被"恶性通胀年份剔除"规则移出了回报统计。因此本文的历史分布是**存活者的分布**，左尾比真实的全球经验更薄——"150 年里约三分之一的十年窗口下跌"是对这 16 国而言的下界式陈述。这个偏差与第 2 条同向：真实区间更宽、真实左尾更厚，本文报告的每一个买房胜率都应读作偏高而非偏低。
