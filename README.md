...
python3 analysis/lookup_table.py          # 第8.4节: 速查表组(表A 判定 + 表B 代价, 含每格 95% 区间与估值修正)
...
- **2026 年中国**（论文第 8 节）：房价下跌 + 按揭 3.1% + 居民投资收益率塌缩，
  ...
  分城市结果与分人群建议之外，8.4 节把全部结论压缩成**一组共用格子的速查表**（纵轴租金收益率、横轴持有期）：
  表 A 给判定（租 / 中性 / 买），表 B 给同一格在历史中位、横盘、日本路径下的财富差；2026 年的读者只需查这一组。
...
查表顺序（论文 8.4 节）：先过四条红线；再查表 A，"租"或"买"即结束；"中性"就查表 B 的同一格，
看日本路径那个数折成你的首付能否承受，能承受且非金融因素明确站在买这边才买。
